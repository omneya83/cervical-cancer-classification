...
imagefolder='D:\Drive 1\Cropped cervical cancer Sky\Segmented Images';
...
%% to run mobilenet
%% to run efficientnet
layer = fullyConnectedLayer(numClasses,Name="new_fc");
lgraph = replaceLayer(lgraph,"Logits",layer);
layer2=classificationLayer('Classes',classNamesnew );

lgraph = replaceLayer(lgraph,"ClassificationLayer_Logits",layer2);


%% remove layers and add self attention
% lgraph = removeLayers(lgraph, ["new_fc" "Logits_softmax" "classoutput"]);
...
% lgraph = connectLayers(lgraph,'global_average_pooling2d_1','flatten');
